...
public class WebGlEmulator {
    ...
    public void drawArrays(WebGlProgramEmulator webGlProgramEmulator) {
        if (canvas == null) {
            return;
        }
        double canvasWidth = canvas.getWidth();
    ...
    private void drawLine(GraphicsContext gc, VertexShader vertexShader, Paint paint, Vertex vertexA, Vertex vertexB) {
        Vertex4 clipA = vertexShader.runShader(vertexA);
        Vertex4 clipB = vertexShader.runShader(vertexB);
    ...
    private void drawTriangle(GraphicsContext gc, VertexShader vertexShader, Paint paint, Vertex vertexA, Vertex vertexB, Vertex vertexC) {
        Vertex4 clipA = vertexShader.runShader(vertexA);
        Vertex4 clipB = vertexShader.runShader(vertexB);
        Vertex4 clipC = vertexShader.runShader(vertexC);
