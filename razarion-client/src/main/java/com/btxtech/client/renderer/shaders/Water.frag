...
vec3 bumpMapNorm(float scale) {
    ...
    float bm1 = texture2D(uBm, vWorldVertexPosition.xy * scale + vec2(0.5, 0.5)).r;
    float bm1Up = texture2D(uBm, vWorldVertexPosition.xy * scale + vec2(0.5, 0.5) + vec2(0.0, uBmOnePixel)).r;
    float bm1Right = texture2D(uBm, vWorldVertexPosition.xy * scale + vec2(0.5, 0.5) + vec2(uBmOnePixel, 0.0)).r;
...
vec3 _bumpMapNorm(float scale) {
    vec3 normal = normalize(vVertexNormal);
    vec3 tangent = normalize(vVertexTangent);
    vec3 binormal = cross(normal, tangent);

    vec2 texCoord = vWorldVertexPosition.xy * scale  + vec2(0, animation);

    float bm0 = texture2D(uBm, texCoord).r;
    float bm0Up = texture2D(uBm, texCoord + vec2(0.0, uBmOnePixel)).r;
    float bm0Right = texture2D(uBm, texCoord + vec2(uBmOnePixel, 0.0)).r;

    float bm1 = texture2D(uBm, texCoord + vec2(0.1, 0.1)).r;
    float bm1Up = texture2D(uBm, texCoord + vec2(0.1, 0.1) + vec2(0.0, uBmOnePixel)).r;
    float bm1Right = texture2D(uBm, texCoord + vec2(0.1, 0.1) + vec2(uBmOnePixel, 0.0)).r;
    ...
    return normalize(normal + uBmDepth * (bump0Vector + bump1Vector));
}
