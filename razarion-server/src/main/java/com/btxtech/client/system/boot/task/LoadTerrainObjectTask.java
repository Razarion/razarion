...
public class LoadTerrainObjectTask extends AbstractStartupTask {
    ...
    protected void privateStart(final DeferredStartup deferredStartup) {
        ...
        terrainServiceCaller.call(new RemoteCallback<Collection<TerrainObject>>() {
            @Override
            public void callback(Collection<TerrainObject> terrainObjects) {
                terrainObjectService.setTerrainObjects(terrainObjects);
                deferredStartup.finished();
            }
        }, new ErrorCallback() {
            @Override
            public boolean error(Object message, Throwable throwable) {
                logger.log(Level.SEVERE, "loadTerrainObjects failed: " + message, throwable);
