...
public class GameUiControlConfigPersistenceImpl implements GameUiControlConfigPersistence {
    ...
    private List<SceneConfig> setupTower() {
        ...
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setStartPointPlacerConfig(baseItemPlacerConfig).setBotConfigs(botConfigs).setWait4QuestPassedDialog(true));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> setupParticle() {
        List<SceneConfig> sceneConfigs = new ArrayList<>();
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(new ViewFieldConfig().setToPosition(new DecimalPosition(200, 200)).setCameraLocked(false)).setRemoveLoadingCover(true));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> useInventoryItem() {
        ...
        buildupItemTypeCount.put(BASE_ITEM_TYPE_BULLDOZER, 1);
        ConditionConfig startConditionConfig = new ConditionConfig().setConditionTrigger(ConditionTrigger.SYNC_ITEM_CREATED).setComparisonConfig(new ComparisonConfig().setTypeCount(buildupItemTypeCount));
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setStartPointPlacerConfig(baseItemPlacerConfig).setQuestConfig(new QuestConfig().setTitle("Platzieren").setDescription("Start").setConditionConfig(startConditionConfig)).setWait4QuestPassedDialog(true));
    ...
    private List<SceneConfig> harvest() {
        ...
        sceneConfigs.add(new SceneConfig().setStartPointPlacerConfig(baseItemPlacerConfig).setViewFieldConfig(viewFieldConfig).setResourceItemTypePositions(resourceItemTypePositions).setQuestConfig(questConfig).setWait4QuestPassedDialog(true));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> kilHumanBase() {
        ...
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setStartPointPlacerConfig(baseItemPlacerConfig).setQuestConfig(new QuestConfig().setConditionConfig(conditionConfig).setTitle("Platzieren").setDescription("Platzieren")).setWait4QuestPassedDialog(true));
        // Setup killer bot
        List<BotConfig> botConfigs = new ArrayList<>();
        List<BotEnragementStateConfig> botEnragementStateConfigs = new ArrayList<>();
        List<BotItemConfig> botItems = new ArrayList<>();
    ...
    private List<SceneConfig> kilEnemyBotBase() {
        ...
        // Camera
        ViewFieldConfig viewFieldConfig = new ViewFieldConfig().setToPosition(new DecimalPosition(243, 90)).setCameraLocked(false);
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setBotConfigs(botConfigs).setBotKillOtherBotCommandConfigs(botKillOtherBotCommandConfigss));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> setupPickBox() {
        ...
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setBoxItemPositions(boxItemPositions).setRemoveLoadingCover(true));
    ...
    private List<SceneConfig> findEnemyBase() {
        ...
        // div
        ViewFieldConfig viewFieldConfig = new ViewFieldConfig().setToPosition(new DecimalPosition(104, 32)).setCameraLocked(false);
        sceneConfigs.add(new SceneConfig().setGameTipConfig(gameTipConfig).setViewFieldConfig(viewFieldConfig).setBotConfigs(botConfigs).setScrollUiQuest(scrollUiQuest).setWait4QuestPassedDialog(true));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> setupAttack() {
        ...
        // div
        ViewFieldConfig viewFieldConfig = new ViewFieldConfig().setToPosition(new DecimalPosition(104, 32)).setCameraLocked(false);
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setBotConfigs(botConfigs).setBotAttackCommandConfigs(botAttackCommandConfigs).setRemoveLoadingCover(true));
        return sceneConfigs;
    }
    ...
    private List<SceneConfig> demolitionVisualization() {
        ...
        sceneConfigs.add(new SceneConfig().setRemoveLoadingCover(true).setViewFieldConfig(viewFieldConfig).setBotConfigs(botConfigs).setBotKillOtherBotCommandConfigs(botKillOtherBotCommandConfigs).setWait4QuestPassedDialog(true));
        return sceneConfigs;
    }
    ...
    private void addBotMoveScene(List<SceneConfig> sceneConfigs) {
        ...
        sceneConfigs.add(new SceneConfig().setViewFieldConfig(viewFieldConfig).setBotMoveCommandConfigs(botMoveCommandConfigs).setIntroText("Folge mir zum Vorposten"));
    }
    ...
    private void addUserMoveScene(List<SceneConfig> sceneConfigs) {
        ...
        sceneConfigs.add(new SceneConfig().setQuestConfig(new QuestConfig().setTitle("Fahre zu Vorposten").setDescription("Folge Kenny und Fahre zum Vorposten. Bewege Deine Einheit zum markierten Bereich").setXp(1).setConditionConfig(conditionConfig)).setGameTipConfig(gameTipConfig).setWait4LevelUpDialog(true));
    }
    ...
    private void addFindEnemyBase(List<SceneConfig> sceneConfigs) {
        ...
        sceneConfigs.add(new SceneConfig().setGameTipConfig(gameTipConfig).setViewFieldConfig(viewFieldConfig).setScrollUiQuest(scrollUiQuest).setWait4QuestPassedDialog(true).setBotHarvestCommandConfigs(botHarvestCommandConfigs));
    }
    ...
    private void addBoxSpawnTask(List<SceneConfig> sceneConfigs) {
        ...
        sceneConfigs.add(new SceneConfig().setGameTipConfig(gameTipConfig).setQuestConfig(new QuestConfig().setXp(1).setTitle("Benutze Inventar").setDescription("Platziere die Militäreinheiten vom Inventar").setConditionConfig(conditionConfig)).setWait4QuestPassedDialog(true).setBotMoveCommandConfigs(botMoveCommandConfigs));
    }
    ...
    private void addWaitForDeadTask(List<SceneConfig> sceneConfigs) {
        sceneConfigs.add(new SceneConfig().setWaitForBaseLostDialog(true));
    }
    ...
    private void addNpcAttackTowerCommand(List<SceneConfig> sceneConfigs) {
        ...
        sceneConfigs.add(new SceneConfig().setIntroText("Komm, greiffen wir an!").setBotAttackCommandConfigs(botAttackCommandConfigs).setDuration(5000).setViewFieldConfig(new ViewFieldConfig().setToPosition(new DecimalPosition(190, 242)).setSpeed(50.0)));
    }
