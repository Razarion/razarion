...
public final class JsUtils {
    ...
    public static native boolean isNull(JSObject obj);
    ...
    @JSBody(params = {"intArray"}, script = "var result = []; for (var i = 0; i < intArray.length; i++) { result[i] = intArray[i]; } return result;")
    public static native JsArray<Integer> convertIntArray(int[] intArray);

    @JSBody(params = {"doubleArray"}, script = "var result = []; for (var i = 0; i < doubleArray.length; i++) { result[i] = doubleArray[i]; } return result;")
    public static native JsArray<Double> convertDoubleArray(double[] doubleArray);
