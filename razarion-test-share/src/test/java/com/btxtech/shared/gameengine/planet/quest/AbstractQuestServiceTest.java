...
public abstract class AbstractQuestServiceTest extends BaseBasicTest {
    ...
    protected void assetQuestProgressCountDownload(String userId, int expectedCount, String expectedBotBasesInformation) {
    ...
    protected void assetQuestProgressTypeCountGameLogicListener(String userId, String expectedBotBasesInformation, int... expected) {
        List<QuestProgressInfo> questProgressInfos = getTestGameLogicListener().getQuestProgresses().get(userId);
        Assert.assertEquals(1, questProgressInfos.size());
    ...
    protected void assetQuestProgressTypeCountDownload(String userId, String expectedBotBasesInformation, int... expected) {
        QuestProgressInfo questProgressInfo = getQuestService().getQuestProgressInfo(userId);
        Assert.assertNotNull(questProgressInfo);
        assetQuestProgressTypeCount(expectedBotBasesInformation, questProgressInfo, expected);
    }

    protected void assertQuestProgressPositionDownload(String userId, Integer secondsRemaining, Integer secondsRemainingDelta, int... expected) {
        QuestProgressInfo questProgressInfo = getQuestService().getQuestProgressInfo(userId);
        Assert.assertNotNull(questProgressInfo);
        assertQuestProgressPosition(questProgressInfo, secondsRemaining, secondsRemainingDelta, expected);
    }

    protected void assertQuestProgressPositionGameLogicListener(String userId, Integer secondsRemaining, Integer secondsRemainingDelta, int... expected) {
        List<QuestProgressInfo> questProgressInfos = getTestGameLogicListener().getQuestProgresses().get(userId);
        Assert.assertEquals(1, questProgressInfos.size());
    ...
    protected void assertQuestProgressPositionGameLogicListenerFirst(String userId, Integer secondsRemaining, Integer secondsRemainingDelta, int... expected) {
        List<QuestProgressInfo> questProgressInfos = getTestGameLogicListener().getQuestProgresses().get(userId);
        assertQuestProgressPosition(questProgressInfos.get(0), secondsRemaining, secondsRemainingDelta, expected);
        getTestGameLogicListener().getQuestProgresses().remove(userId, questProgressInfos.get(0));
    }

    protected void assertQuestProgressPositionGameLogicListenerNone(String userId) {
        List<QuestProgressInfo> questProgressInfos = getTestGameLogicListener().getQuestProgresses().get(userId);
