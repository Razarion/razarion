...
//                orca1.solve();
    ...
    @Test
    public void test2Movable() {
        SyncPhysicalMovable syncPhysicalMovable = GameTestHelper.createSyncPhysicalMovable(2.0, TerrainType.LAND, new DecimalPosition(40, 40), new DecimalPosition(10, 0), new DecimalPosition(10, 0), 10.0);
        List<SyncPhysicalMovable> shifities = new ArrayList<>();
        SyncPhysicalMovable shifty = GameTestHelper.createSyncPhysicalMovable(2.0, TerrainType.LAND, new DecimalPosition(44.5, 40), null, null, 10.0);
        PathingServiceUtil.setupPushAwayVelocity(syncPhysicalMovable, shifty);
        shifities.add(shifty);
    ...
    private void display(Orca orca, SyncPhysicalMovable syncPhysicalMovable, List<ObstacleSlope> obstacles, List<SyncPhysicalMovable> otherSyncPhysicalMovables) {
        TestGuiDisplay.show(new AbstractTestGuiRenderer() {
            @Override
            protected void doRender() {
                if (obstacles != null) {
                ...
                if (otherSyncPhysicalMovables != null) {
                    otherSyncPhysicalMovables.forEach(syncPhysicalMovable1 -> {
                        strokeSyncPhysicalMovable(syncPhysicalMovable1, 0.05, Color.GREEN);
                    });
                }
                orca.getDebugObstacles_WRONG().forEach(obstacleSlope -> {
                    strokeObstacleSlope(obstacleSlope, 0.4, new Color(1, 1, 0, 0.8));
