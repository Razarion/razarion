...
public class SyncItemTypeKilledCountTest extends AbstractQuestServiceTest {
    ...
    public void killCount() {
        ...
        QuestConfig questConfig = new QuestConfig().id(9001).conditionConfig(new ConditionConfig().conditionTrigger(ConditionTrigger.SYNC_ITEM_KILLED).comparisonConfig(new ComparisonConfig().count(3)));
    ...
    public void killCountBotId() {
        ...
        HumanBaseContext humanBaseContext = createHumanBaseBFA();
        // Create and activate quest
        QuestConfig questConfig = new QuestConfig().id(9002).conditionConfig(new ConditionConfig().conditionTrigger(ConditionTrigger.SYNC_ITEM_KILLED).comparisonConfig(new ComparisonConfig().count(1).botIds(Collections.singletonList(botBase3.getBotId()))));
        getQuestService().addQuestListener(createQuestListener());
        getQuestService().activateCondition(humanBaseContext.getPlayerBaseFull().getUserId(), questConfig);
