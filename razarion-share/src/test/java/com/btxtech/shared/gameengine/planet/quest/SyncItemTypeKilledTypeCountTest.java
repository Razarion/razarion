...
public class SyncItemTypeKilledTypeCountTest extends AbstractQuestServiceTest {
    ...
    public void killTypeCount() {
        ...
        QuestConfig questConfig = new QuestConfig().id(9001).conditionConfig(new ConditionConfig().conditionTrigger(ConditionTrigger.SYNC_ITEM_KILLED).comparisonConfig(new ComparisonConfig().typeCount(typeCount)));
    ...
    public void killTypeCountBotId() {
        ...
        typeCount.put(FallbackConfig.FACTORY_ITEM_TYPE_ID, 1);
        typeCount.put(FallbackConfig.BUILDER_ITEM_TYPE_ID, 1);
        QuestConfig questConfig = new QuestConfig().id(9001).conditionConfig(new ConditionConfig().conditionTrigger(ConditionTrigger.SYNC_ITEM_KILLED).comparisonConfig(new ComparisonConfig().typeCount(typeCount).botIds(Arrays.asList(botFactory.getBotId(), botBuilder.getBotId()))));
        getQuestService().addQuestListener(createQuestListener());
        getQuestService().activateCondition(humanBaseContext.getPlayerBaseFull().getUserId(), questConfig);
