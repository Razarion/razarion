...
public class ItemTypePositionComparison extends AbstractSyncItemComparison /*implements TimeAware*/ {
    ...
    public void init(Map<BaseItemType, Integer> itemTypes, PlaceConfig placeConfig, Integer time, boolean addExistingItems, UserContext examinee) {
    ...
    private void addInitial() {
        PlayerBase playerBase = baseItemService.getPlayerBase(examinee);
        if (playerBase == null) {
            return;
        }
...
//    }
    ...
    private boolean checkRegion(SyncBaseItem syncBaseItem) {
        return placeConfig == null || syncBaseItem.getSyncPhysicalArea().contains(placeConfig);
    }
