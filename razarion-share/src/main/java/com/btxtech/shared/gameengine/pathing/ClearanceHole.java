...
public class ClearanceHole {
    ...
    public static class AngleSegment {
        ...
        public boolean overlaps(AngleSegment segment) {
            double deltaMiddle = Pathing.deltaAngle(middle, segment.middle);
        ...
        public AngleSegment combines(AngleSegment segment) {
            ...
            double newHalf = Pathing.deltaAngle(side2, side1) / 2.0;
            return new AngleSegment(Pathing.correctAngle(side1 + newHalf), newHalf);
        }

        public boolean isInside(double angle) {
            double delta = Pathing.deltaAngle(middle, angle);
            return delta < half;
        }

        public double getNearestSide(double angle) {
            double angle1 = Pathing.correctAngle(middle + half);
            double angle2 = Pathing.correctAngle(middle - half);

            if (Pathing.deltaAngle(angle, angle1) < Pathing.deltaAngle(angle, angle2)) {
                return angle1;
            } else {
                return angle2;
            }
