...
public class Unit {
    ...
    private final static double LOOK_AHEAD_DISTANCE = LOOK_AHEAD_TICKS * SPEED * PathingService.FACTOR;
    private final static double BYPASS_FACTOR = 2.0 * SPEED * PathingService.FACTOR;
    ...
    public DecimalPosition getDesiredPosition() {
        ...
        if (velocity != null) {
            desiredPosition =
                    desiredPosition.add(velocity.multiply(PathingService.FACTOR));
        }
    ...
    public void setupForTick(List<Unit> units, long tickCount) {
        ...
        if (destination != null) {
            ...
            if (Math.abs(deltaAngle) > ANGULAR_VELOCITY * PathingService.FACTOR) {
                debugHelper.append("angle too big");
                double possibleAngle = PathingService.correctAngle(angle + Math.signum(deltaAngle) * ANGULAR_VELOCITY * PathingService.FACTOR);
            ...
            double speed;
            if (Math.abs(originalSpeed - possibleSpeed) > ACCELERATION * PathingService.FACTOR) {
                if (originalSpeed < possibleSpeed) {
                    debugHelper.append("accelerate");
                    speed = originalSpeed + ACCELERATION * PathingService.FACTOR;
                } else {
            ...
            double acceleration = ACCELERATION * PathingService.FACTOR;
