...
public class Water {
    ...
    public InterpolatedTerrainTriangle getInterpolatedVertexData(Line3d worldPickRa) {
        DecimalPosition absoluteXY = worldPickRa.calculatePositionOnHeightLevel(level).toXY();
        if (!calculateAabb().contains(absoluteXY)) {
            return null;
        }
