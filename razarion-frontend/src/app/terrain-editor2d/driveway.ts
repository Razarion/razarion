...
export class Driveway {
  ...
  constructor(public drivewayId: number, private slope: Slope) {
  }
  ...
  append(polygon: Feature<Polygon, any>) {
    ...
    console.info(`indexesToAdd: ${indexesToAdd.length}`)
  ...
  static mergePaths(polygonLength: number, path1: number[], path2: number[]): number[] {
    ...
    console.log(`mergePathsCounterclockwise ${emptyStartIndex} ${merged}`)
    return merged;
  }
